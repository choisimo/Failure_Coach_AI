...
1. IRL 비활성 상태와 기존 동작 완전 동일 (회귀 테스트 통과).
2. IRL 활성 상태에서 응답 메타데이터가 프론트에 반영되고 UI가 정상 렌더.
3. 거부 시나리오, 정책 전환, 재생성, 피드백 이벤트가 사양대로 동작.
4. 텔레메트리 대시보드에서 지표가 실시간 집계되고 알람 정상 작동.
